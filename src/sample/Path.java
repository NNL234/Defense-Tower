    ...
    public  List setPath() {
            ...
            int cellY = (int)(spawner.getPos().getY()/110);
                ...
                while (!(cellX == dest.getPos().getX()/110 && cellY == dest.getPos().getY()/110)) {
                    ...
                    if (cellX +1 <6 && !checkedMap[cellY][cellX + 1] && Setting.map[cellY][cellX + 1] == 1) {
                        cellX++;
                    } else if (cellX - 1 >=0 && !checkedMap[cellY][cellX - 1] && Setting.map[cellY][cellX - 1] == 1) {
                        cellX--;
                    } else if (cellY - 1 >= 0 && !checkedMap[cellY - 1][cellX] && Setting.map[cellY - 1][cellX] == 1) {
                        cellY--;
                    } else if (cellY + 1 < 6 && !checkedMap[cellY + 1][cellX] && Setting.map[cellY + 1][cellX] == 1) {
                        cellY++;
                    }
